...
public func classifyFallibles<T>(array: [Fallible<T>]) -> (successes: [T], failures: [ErrorType]) {
    return array.reduce((successes: [] as [T], failures: [] as [ErrorType])) { (tuple, elem) in
...
/// version of an API.
//
// N.B. This is not yet an ErrorType because ErrorType doesn't support userInfo yet.
public enum FallibleError: Int {
    ...
    static func errorsFromError(error: ErrorType?) -> [NSError] {
    ...
    static func errorFromErrors(errors: [ErrorType]) -> NSError? {
        let flatErrors: [NSError] = lazy(errors).map { errorsFromError($0) }.reduce([], combine: +)
        ...
        if flatErrors.count > 1 {
            ...
            return flatErrors.first.map { $0 as NSError }
        }
    }
}
